let currentWiggle = 0;
let currentWiggleName = ["no wiggle","wiggle 1","wiggle 2","wiggle 3"];
...
function changeWiggle(direction) {
  ...
  if (currentWiggle < 0) {
    currentWiggle = 3;
  } else if (currentWiggle > 3) {
    currentWiggle = 0;
  }

  document.getElementById('wiggleValue').textContent = currentWiggleName[currentWiggle];
...
let wiggle = false;
let wiggleType = 0;
let bgColorPicker, strokeColorPicker, strokeColorPicker2; 
...
function setup() {
  ...
  bgColorPicker.parent('bgColorPicker'); 
  // strokeColorPicker = createColorPicker('black');
  // strokeColorPicker.position(50, canvasSize + 50);
  // strokeColorPicker2 = createColorPicker('red');
  // strokeColorPicker2.position(100, canvasSize + 50);
  // sizeSlider = createSlider(1, 2, 2, 0); 
  // sizeSlider.position(160, canvasSize + 50);
}

// function toggleWiggle(type) {
//   wiggle = !wiggle;
//   wiggleType = type; // Set the current wiggle type
// }
...
function drawVideoBackground() {
  ...
  scale(-1, 1);
  tint(255, 255, 255, 50);
  image(myCapture, 0, 0, videoWidth, videoHeight);
  pop();
}
...
function drawConnectors(landmarks, connectorSet) {
  ...
  if (landmarks) {
    ...
    for (let i = 0; i < nConnectors; i++) {
      ...
      const skipIndices = [46,53,52,65,55, 285,295,282,283,276];
